...
export function ClipCard({ clip, recommended = false }: { clip: Clip; recommended?: boolean }) {
  ...
  return (
    <div className="group flex flex-col">
      ...
      <div
        className="relative aspect-[9/16] overflow-hidden rounded-2xl bg-ink-950 ring-1 ring-white/10 shadow-rim transition duration-200 ease-premium group-hover:-translate-y-0.5 group-hover:ring-brand/50 group-hover:shadow-lift"
      ...
      >
        ...
        {recommended && (
          <span className="absolute left-2 top-2 z-10 inline-flex items-center gap-1 rounded-lg bg-brand px-2 py-1 text-[11px] font-semibold text-white shadow-glow">
            <svg viewBox="0 0 24 24" className="h-3 w-3 fill-current" aria-hidden><path d="M12 2l2.9 6.3 6.6.6-5 4.4 1.5 6.5L12 16.9 5.9 19.8 7.4 13.3l-5-4.4 6.6-.6z" /></svg>
            Recommended
          </span>
        )}
        <span className="absolute right-2 top-2 z-10 rounded-lg bg-black/60 px-2 py-1 font-mono text-xs font-medium tabular-nums text-white backdrop-blur">
